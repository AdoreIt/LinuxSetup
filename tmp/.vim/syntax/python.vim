...
syn keyword pythonStatement	return try with self
syn keyword pythonStatement	global assert
syn keyword pythonStatement	lambda yield

syn match   pythonClassDef	/^\s*\%(class\)/
  \ nextgroup=pythonFunction skipwhite
syn match   pythonDefStatement	/^\s*\%(def\)/
  \ nextgroup=pythonFunction skipwhite
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  HiLink pythonStatement	pythonStatement
  HiLink pythonDefStatement	PreCondit
  HiLink pythonFunction		Function
  HiLink pythonConditional	Conditional
  HiLink pythonRepeat		Repeat
  HiLink pythonString		String
  HiLink pythonRawString	String
  HiLink pythonEscape		Special
  HiLink pythonOperator	        pythonOperator
  HiLink pythonPreCondit	PreCondit
  HiLink pythonComment		Comment
  HiLink pythonTodo		pythonTodo
  HiLink pythonDecorator	Define
  if exists("python_highlight_numbers")
    HiLink pythonNumber	Number
  endif
  if exists("python_highlight_builtins")
    HiLink pythonBuiltin	pythonBuiltinObj "Function
  endif
  if exists("python_highlight_exceptions")
    HiLink pythonException	pythonException
  endif
  if exists("python_highlight_space_errors")
